...
exports.newUser = async (req, res, next) => {
  try {
    ...
    const newUser = await UserModel.saveUserDetails({
      firstName,
      lastName,
      email,
      password,
    ...
    console.log(error);
    return next(new AppError("User registration error", 500));
  }
};
...
exports.userRegister = async (req, res) => {
  try {
    ...
    const varifyAddress = await verifyWalletAddress(
      req.body.address,
      req.body.signature,
    );
    ...
    if (isAddressBlocked(req.body.address)) {
      const { response, statusCode } = forbiddenResponse(
        "This address is blocked by admin",
      );
    ...
    if (getUsersaddress.length > 0) {
      ...
      if (req.body.referral_address) {
        ...
        let getRefUsersDetails = await UserModel.getUserDetailsByAddress(
          req.body.referral_address,
        );
...
exports.SingalClaimReward = async (req, res) => {
  try {
    ...
    if (rewardCheck[0].isClaimAvailable == 0) {
      ...
      const myToken =
        parseFloat(stakingQuantity[0].reward_token) *
        parseFloat(stakingQuantity[0].remaining_quantity);
      if (myToken == 0) {
        return res.status(200).send({
          success: false,
...
exports.SellPlan = async (req, res) => {
  try {
    ...
    req.body.reward_token =
      parseFloat(checkSellPlan[0].reward_token) *
      parseFloat(checkSellPlan[0].remaining_quantity);
...
exports.WithdrawCrypto = async (req, res) => {
  try {
    ...
    if (
      ...
      parseFloat(totalBalanceDetail[0].total_balance)
    ) {
      return res.status(200).send({
        success: false,
    ...
    if (data.length > 0) {
      for (let i = 0; i < data.length; i++) {
        try {
          ...
          const clientAddress = await web3.eth.abi.decodeParameter(
            "address",
            newhash.logs[0].topics[2],
          ); // client desposit address
          let busdToken = await web3.eth.abi.decodeParameter(
            "uint",
            newhash.logs[0].data,
          ); // token more then 0
          ...
          if (
            from.toUpperCase() == data[i].from_address.toUpperCase() &&
            ...
            if (getUsersaddress[0].referral_id > 0) {
              let getReferralUser = await UserModel.getReferralUser(
                getUsersaddress[0].referral_id,
              );
          ...
          logger.error(
            `Error processing deposit for transaction ID ${data[i].id}:`,
            error,
          );
          await UserModel.userBalanceReject({ id: data[i].id });
        }
