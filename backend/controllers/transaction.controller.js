...
exports.createDeposit = async (req, res) => {
  try {
    ...
    await UserModel.saveDepositBUSDDetails({
      ...body,
      to_address: require("../config").clientDepositAddress,
      token: parseFloat(body.busd_amount) * 10,
    });
    return res
      .status(200)
      .send({
        success: true,
        msg: "Deposit recorded. Awaiting blockchain confirmation.",
      });
  } catch (_e) {
...
exports.list = async (req, res) => {
  try {
    const data = await UserModel.getTransactionHistory({
      user_id: req.user_id,
    });
